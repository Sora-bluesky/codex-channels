...
>
> A. ブリッジが実行中ターンを見つけられる場合、テキストは保留入力として渡されます。
> Codex 側では、現在の処理に続く追加入力として扱われます。
> 実行中ターンがある間、添付は保留入力にできません。
> 添付は現在のターンが終わってから送ってください。
> ブリッジ再起動後は、実行中ターンを復元できないことがあります。
> その場合は、現在の処理が終わってから再送してください。
