...
- `message.from.id` を手で調べずに pairing する
...
このリポジトリを Codex で開き、ローカル `plugin` の `remotty` を入れてください。
...
先にブリッジが止まっていることを確認します。その上で、次を実行します。

```text
/remotty-pair
```

このコマンドは、まずローカル端末へ 12 文字の一時 code を出します。Telegram 側では、許可したいアカウントから `/pair <code>` を送ってください。コマンドはその一致メッセージを待ち、対象の `sender_id` と `chat_id` を端末へ出した上で、その sender をローカル allowlist へ追加します。
...
互換経路として、`cargo run` で直接起動することもできます。
...
`Rust` の単独 CLI は、互換用として引き続き残しています。`plugin` の裏では、次の CLI を呼びます。

- `/remotty-configure` -> `cargo run -- telegram configure --config bridge.toml`
- `/remotty-pair` -> `cargo run -- telegram pair --config bridge.toml`
- `/remotty-policy-allowlist` -> `cargo run -- telegram policy allowlist --config bridge.toml`
- `/remotty-status` -> `cargo run -- service status`
- `/remotty-live-env-check` -> `cargo run -- telegram live-env-check`
- `/remotty-smoke-approval-accept` -> `cargo run -- telegram smoke approval accept --config bridge.toml`
- `/remotty-smoke-approval-decline` -> `cargo run -- telegram smoke approval decline --config bridge.toml`
...
値そのものをチャットへ貼らないでください。端末全体のスクリーンショットも避けてください。
...
`LIVE_WORKSPACE` は、専用の作業用フォルダを指してください。
そのフォルダには、事前に `.remotty-live-smoke-ok` という空ファイルを置く必要があります。
この印がないフォルダには、手動スモークは書き込みません。
